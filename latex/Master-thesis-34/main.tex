...
%%%% TODO: metti la citazione coatta 

\begin{abstract}
As Graph Neural Networks (GNNs) gain traction in real-world applications like computational biology, natural language processing (NLP, \cite{wu2022-NLP-graph}), and computer security (\cite{zhou2018-GraphNN}), the need for explainability becomes apparent due to their inherent nature of \say{black-box} deep learning models. Current approaches for interpreting GNN predictions have predominantly concentrated on individuating subgraphs that are especially relevant for a particular prediction, delving into crucial graph substructures and node features. As a result, the generated explanations are painstakingly customized for each instance. 
...
%The distinctive approach of interpreting each instance in isolation falls short of offering a comprehensive understanding of the learned GNN model. This limitation hampers its generalizability and impedes its utility in inductive scenarios. Additionally, as it is primarily crafted for explaining individual instances, it faces challenges when it comes to naturally explaining a set of instances, such as graphs within a specific class.

%However, compared with image and text domains, the explainability of graph models are less explored, which is critical for understanding deep graph neural networks. Recently, several approaches are proposed to explain the predictions of GNNs [44], such as XGNN [45], GNNExplainer [46], PGExplainer [47], and SubgraphX [48], etc. 
...
Chapter \ref{chap:2-background}: \say{Background}, provides an in-depth review of the related literature, covering topics related to deep learning and Graph Neural Networks (GNNs), exaplainable artificial intelligence and counterfactual explanations.
Chapter \ref{chap:3-CFPG}: \say{Parametrized Counterfactual Explanations for Node Classification in Graph Neural Networks}, outlines the proposed methodologies for generating counterfactual explanations for GNNs, including algorithmic details and rationales.
Chapter \ref{chap:4-expRes}: \say{Experimental setup} presents the experimental setup, datasets, evaluation metrics, and results obtained from assessing the effectiveness of the proposed methodologies.
Chapter \ref{chap:5-conclusions}: \say{Conclusions}, summarizes the contributions of the thesis, discusses limitations, and outlines potential directions for future research in this domain.
...
%    \item \textbf{Chapter 4: Experimental setup} presents the experimental setup, datasets, evaluation metrics, and results obtained from assessing the effectiveness of the proposed methodologies.
%    \item \textbf{Chapter 5: Conclusions } summarizes the contributions of the thesis, discusses limitations, and outlines potential directions for future research in this domain.
%\end{itemize}
...
\begin{figure}
    ...
    \caption{\textit{Graph of countefactual XAI publications in literature from the Scopus\protect\footnotemark[1] and Web of Science (WoS\protect\footnotemark[2]) databases. The graph contains the most popular author keywords for counterfactual explanation \mycite{stepin2021-xai-cf-contrative-survey}. This topic is often investigated in the context of causation (pay attention to such keywords as \protect\say{causation}, \protect\say{causal inference} or \protect\say{causal models}) as well as cognitive science (as reflected by the keywords \protect\say{imagination}, \protect\say{reasoning}, etc.) and AI (\protect\say{machine learning}, \protect\say{data models}, \protect\say{black-box models}).}}
    \label{fig:br.xai.xai-graph}
\end{figure}

\footnotetext[1]{\url{https://www.elsevier.com/en-gb/solutions/scopus}}
\footnotetext[2]{\url{https://clarivate.libguides.com/webofscienceplatform/coverage}}
...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrr}
        \hline
         &
          \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}BAshapes\\ (syn1)\end{tabular}} &
          \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}BAcommunity\\ (syn2)\end{tabular}} &
          \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}TreeCycles\\ (syn3)\end{tabular}} &
          \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}TreeGrids\\ (syn4)\end{tabular}} \\ 
        \hline
...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrrrr}
        \hline
        \multicolumn{1}{c}{CFPGbase} &
          \multicolumn{1}{c}{optimizer} &
          \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}learning\\ rate\end{tabular}} &
          \multicolumn{1}{c}{$\xi$} &
          \multicolumn{1}{c}{$\beta$} &
          \multicolumn{1}{c}{$\gamma$} & 
          \multicolumn{1}{c}{$(\tau_1,\tau_2)$} \\ 
        \hline
...
%\begin{itemize}
%    \item syn1 indices of expl. labeled nodes: $[300,700)$ every 5;
%    \item syn2 indices of expl. labeled nodes: $[300,700) \cup [1000,1400)$ every 5;
%    \item syn3 - indices of expl. labeled nodes: $[511,871)$ every 6;
%    \item syn4 - indices of expl. labeled nodes: $[511,1231)$ every 3/9;
%\end{itemize}
