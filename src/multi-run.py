...
params = {
    "syn1" : "--opt Adam --reg-ent 1.0 --reg-cf 1.0 --reg-size 0.1 --heads 3 --hid-gcn 20 --add-att 0.0", # --temp [1.0,1.0]
    "syn2" : "--opt Adam --reg-ent 1.0 --reg-cf 1.0 --reg-size 0.1 --heads 3 --hid-gcn 20 --add-att 0.0", # --temp [1.0,1.0]
    "syn3" : "--opt Adam --reg-ent 1.0 --reg-cf 1.0 --reg-size 0.1 --heads 3 --hid-gcn 20 --add-att 0.0", # --temp [1.0,1.0]
    "syn4" : "--opt Adam --reg-ent 1.0 --reg-cf 1.0 --reg-size 0.1 --heads 3 --hid-gcn 20 --add-att 0.0", # --temp [1.0,1.0] 
}
...
for s in SEEDS:
    ...
    #for d in (d_bar := tqdm(DATASETS, desc=f"[seed {s:03}]> datasets... ", colour="green", disable=False)):
        script_args = f" -E {EXPLAINER} -D {d} -e {EPOCHS} --conv GAT {params[d]} --seed {s} -es 10 "
        suffix_args = f"--prefix rAllOne-Sparsemax0-thres05-allEdges-3GAT-Estop10-Split --log"
